...
public class HunterArrow extends AbstractArrow {
    ...
    protected void onHitEntity(EntityHitResult result) {
        ...
        if (!level().isClientSide()
            ...
            if (!target.isAlive() || target.isDeadOrDying()) {
                improveDrops(serverLevel, target);
            }
        }
    }
     ...
     *
     */
    private void improveDrops(ServerLevel serverLevel, LivingEntity entity) {
        ...
        if (entity.getType() == EntityType.COW && entity instanceof Cow) {
            ...
            if (leather > 0) entity.spawnAtLocation(serverLevel, new ItemStack(Items.LEATHER, leather));
            ...
            if (!sheep.isSheared()) {
                ...
                if (woolBlock != null) entity.spawnAtLocation(serverLevel, new ItemStack(woolBlock));
            }

        } else if (entity.getType() == EntityType.CHICKEN && entity instanceof Chicken) {
            int meat = 1;
            int feathers = rng.nextInt(3); // 0–2

            entity.spawnAtLocation(serverLevel, new ItemStack(cooked ? Items.COOKED_CHICKEN : Items.CHICKEN, meat));
            if (feathers > 0) entity.spawnAtLocation(serverLevel, new ItemStack(Items.FEATHER, feathers));
            ...
            if (hide > 0) entity.spawnAtLocation(serverLevel, new ItemStack(Items.RABBIT_HIDE, hide));
