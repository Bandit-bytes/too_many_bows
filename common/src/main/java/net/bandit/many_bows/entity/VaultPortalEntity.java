...
public class VaultPortalEntity extends Entity {
    ...
    protected void readAdditionalSaveData(CompoundTag tag) {
        ...
        this.trackTarget = tag.getBoolean("TrackTarget");
        this.powerMultiplier = tag.getFloat("PowerMultiplier");
        this.portalIndex = tag.getInt("PortalIndex");
        this.hasFired = tag.getBoolean("HasFired");
        this.fireAnimTicks = tag.getInt("FireAnimTicks");
    }
    ...
    protected void addAdditionalSaveData(CompoundTag tag) {
        ...
        tag.putBoolean("TrackTarget", this.trackTarget);
        tag.putFloat("PowerMultiplier", this.powerMultiplier);
        tag.putInt("PortalIndex", this.portalIndex);
        tag.putBoolean("HasFired", this.hasFired);
        tag.putInt("FireAnimTicks", this.fireAnimTicks);
    }
    ...
    public void tick() {
        ...
        if (!hasFired && trackTarget && target != null && target.isAlive()) {
            double bob = Math.sin((this.tickCount + (portalIndex * 3)) * 0.25D) * config.portal_vertical_bob;
            this.setPos(
                    target.getX() + offsetX,
                    target.getY() + offsetY + bob,
                    target.getZ() + offsetZ
            );
        ...
        if (!hasFired) {
            if (--warmupTicks <= 0) {
                ...
                hasFired = true;
                fireAnimTicks = MAX_FIRE_ANIM_TICKS;
        ...
        if (fireAnimTicks > 0) {
            fireAnimTicks--;
            this.entityData.set(FIRE_ANIM_TICKS, fireAnimTicks);
        } else {
            this.discard();
        }
    }
    ...
    private void tickClientParticles(VaultpiercerBowConfig config) {
        ...
        for (int i = 0; i < Math.max(1, config.portal_particle_count / 2); i++) {
            ...
            double y = this.getY() + (Math.sin((tickCount + i) * 0.18D) * 0.04D);
    ...
    private void fireFollowUpArrow(VaultpiercerBowConfig config) {
        ...
        arrow.moveTo(this.getX(), this.getY(), this.getZ(), this.getYRot(), this.getXRot());
        ...
        arrow.shoot(diff.x, diff.y, diff.z, config.follow_up_projectile_velocity, 0.0F);
        ...
        serverLevel.sendParticles(ParticleTypes.END_ROD,
                this.getX(), this.getY(), this.getZ(),
                10, 0.08D, 0.08D, 0.08D, 0.01D);
        serverLevel.sendParticles(ParticleTypes.REVERSE_PORTAL,
                this.getX(), this.getY(), this.getZ(),
                8, 0.04D, 0.04D, 0.04D, 0.0D);
    }
