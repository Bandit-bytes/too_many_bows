...
public class SoulhoardArrow extends AbstractArrow {
    ...
    public SoulhoardArrow(EntityType<? extends SoulhoardArrow> entityType, Level level) {
    ...
    public SoulhoardArrow(Level level, LivingEntity shooter, ItemStack bowStack, ItemStack arrowStack) {
        super(EntityRegistry.SOULHOARD_ARROW.get(), shooter, level, safeArrowStack(arrowStack), safeBowStack(bowStack));
    ...
    public void setHoardedSouls(int souls) {
        ...
        if (clamped > 0) {
            ...
            this.setBaseDamage(((AbstractArrowAccessor) this).manybows$getBaseDamage() + totalBonus);
        }
    }
    ...
    protected void addAdditionalSaveData(ValueOutput out) {
        ...
        out.putInt("HoardedSouls", this.getHoardedSouls());
        out.putFloat("PowerMultiplier", this.powerMultiplier);
        out.putInt("Lifetime", this.lifetime);
        out.putBoolean("ReleasedSkulls", this.releasedSkulls);
        out.putBoolean("SoulLanternEmpowered", this.soulLanternEmpowered);

        if (this.sourceBowId != null) {
            out.putString("SourceBowId", this.sourceBowId.toString());
        }
    }
    ...
    protected void readAdditionalSaveData(ValueInput in) {
        ...
        this.entityData.set(
                DATA_HOARDED_SOULS,
                Math.max(0, in.getInt("HoardedSouls").orElse(this.getHoardedSouls()))
        );
        ...
        this.releasedSkulls = in.getBooleanOr("ReleasedSkulls", this.releasedSkulls);
        this.soulLanternEmpowered = in.getBooleanOr("SoulLanternEmpowered", this.soulLanternEmpowered);

        String sourceId = in.getString("SourceBowId").orElse("");
        if (!sourceId.isBlank()) {
            try {
                this.sourceBowId = UUID.fromString(sourceId);
            } catch (IllegalArgumentException ignored) {
                this.sourceBowId = null;
            }
        }
    }
