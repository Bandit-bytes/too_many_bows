...
public class SoulhoardBow extends ModBowItem {
    ...
    public static int getSoulCount(ItemStack stack) {
        ...
        return Mth.clamp(tag.getInt(SOULS_KEY).orElse(0), 0, getAbsoluteMaxSoulCapacity());
    }
    ...
    public static UUID getOrCreateBowId(ItemStack stack) {
        UUID existing = getBowIdIfPresent(stack);
        if (existing != null) {
            return existing;
        }
        ...
        CompoundTag tag = getCustomTag(stack);
        tag.putString(BOW_ID_KEY, uuid.toString());
    ...
    public static UUID getBowIdIfPresent(ItemStack stack) {
        CompoundTag tag = getCustomTag(stack);
        String id = tag.getString(BOW_ID_KEY).orElse("");
        if (id.isBlank()) {
            return null;
        }
    ...
    public static boolean awardSoulsToMatchingBow(Player player, UUID bowId, int amount) {
        if (player == null || bowId == null || amount <= 0) {
            return false;
        }
