...
public class SolarBow extends ModBowItem {
    ...
    protected Projectile createProjectile(Level level, LivingEntity shooter, ItemStack weaponStack, ItemStack ammoStack, boolean crit) {
        ...
        if (ammoItem instanceof ArrowItem arrowItem
            ...
            arrow = new SolarArrow(level, shooter, weaponStack, ammoStack);
        }
        ...
        if (shooter instanceof Player player) {
            float ranged = getRangedWeaponDamage(level, player, 3.0F);
            arrow.setBaseDamage(ranged / 2.0F);

            applyBowDamageAttribute(arrow, player);
            tryApplyBowCrit(arrow, player, 1.5D);
        }

        if (arrow instanceof SolarArrow sa) {
            sa.setPowerMultiplier(1.0F);
        }
    ...
    protected void shootProjectile(LivingEntity shooter, Projectile projectile, int index,
        ...
        // velocity is (power * 2.5F) -> power = velocity / 2.5F
        if (projectile instanceof SolarArrow sa) {
            sa.setPowerMultiplier(velocity / 2.5F);
        }
    ...
    public void appendHoverText(ItemStack stack,
        ...
        if (Minecraft.getInstance().hasShiftDown()) {
            tooltip.accept(Component.translatable("item.many_bows.solar_bow.tooltip")
                    .withStyle(ChatFormatting.GOLD, ChatFormatting.BOLD));
            tooltip.accept(Component.translatable("item.many_bows.solar_bow.tooltip.ability")
