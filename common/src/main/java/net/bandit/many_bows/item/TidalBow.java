...
public class TidalBow extends ModBowItem {
    ...
    public boolean releaseUsing(ItemStack bowStack, Level level, LivingEntity entity, int chargeTime) {
        try {
            ...
            level.playSound(null, player.getX(), player.getY(), player.getZ(),
                    SoundEvents.BUCKET_EMPTY_AXOLOTL, SoundSource.PLAYERS,
                    0.3F, 1.1F);
    ...
    protected Projectile createProjectile(Level level, LivingEntity shooter, ItemStack weaponStack, ItemStack ammoStack, boolean crit) {
        ...
        if (ammoItem instanceof ArrowItem arrowItem
            ...
            AbstractArrow vanilla = arrowItem.createArrow(level, ammoStack, shooter, weaponStack);
            if (crit) vanilla.setCritArrow(true);

            if (level instanceof ServerLevel serverLevel) {
                EnchantmentHelper.onProjectileSpawned(serverLevel, weaponStack, vanilla, (item) -> {});
            }
            if (shooter instanceof Player player) {
                applyBowDamageAttribute(vanilla, player);
                tryApplyBowCrit(vanilla, player, 1.5D);
            }
            return vanilla;
        }

        TidalArrow arrow = new TidalArrow(level, shooter, weaponStack, ammoStack);
        if (crit) arrow.setCritArrow(true);
        float base = getRangedWeaponDamage(level, shooter);
        arrow.setBaseDamage(base / 2.5F);
    ...
    public void appendHoverText(ItemStack stack,
        ...
        if (isShiftDownSafe()) {
            tooltip.accept(Component.translatable("item.too_many_bows.tidal_bow.tooltip")
                    .withStyle(ChatFormatting.AQUA));
            tooltip.accept(Component.translatable("item.too_many_bows.tidal_bow.tooltip.ability")
                    .withStyle(ChatFormatting.DARK_AQUA));
            tooltip.accept(Component.translatable("item.too_many_bows.tidal_bow.tooltip.legend")
