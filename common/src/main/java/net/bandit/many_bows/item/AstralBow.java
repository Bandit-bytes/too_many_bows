...
public class AstralBow extends ModBowItem {
                    ...
                    if (!projectiles.isEmpty() && level instanceof ServerLevel serverLevel) {
                        for (ItemStack projectileStack : projectiles) {
                            ...
                            if (projectileStack.is(Items.SPECTRAL_ARROW) || projectileStack.is(Items.TIPPED_ARROW)) {
                                ...
                                arrow = new AstralArrow(serverLevel, player, bowStack, projectileStack);
                                if (arrow instanceof AstralArrow astralArrow) {
                                    ...
                                    if (rangedDamageAttr != null) {
                                        ...
                                        if (attrInstance != null) {
                                            ...
                                            astralArrow.setBaseDamage(damage * power);
                                        }
                                    }
                                }
                            }
    ...
    public void appendHoverText(ItemStack stack, TooltipContext context, List<Component> tooltipComponents, TooltipFlag tooltipFlag) {
        if (Screen.hasShiftDown()) {
            tooltipComponents.add(Component.translatable("item.too_many_bows.astral_bow").withStyle(ChatFormatting.AQUA));
            tooltipComponents.add(Component.translatable("item.too_many_bows.astral_bow.tooltip").withStyle(ChatFormatting.DARK_PURPLE));
            tooltipComponents.add(Component.translatable("item.too_many_bows.astral_bow.details").withStyle(ChatFormatting.GRAY));
            } else {
                tooltipComponents.add(Component.translatable("item.too_many_bows.hold_shift").withStyle(ChatFormatting.DARK_GRAY));
            }
        }
    }
