...
public class TorchbearerBow extends ModBowItem {
    ...
    public void inventoryTick(ItemStack stack,
        ...
        boolean hasOrb = !level.getEntitiesOfClass(
                LightOrbEntity.class,
                player.getBoundingBox().inflate(3.0D)
        ).isEmpty();

        if (!hasOrb) {
            LightOrbEntity orb = new LightOrbEntity(EntityRegistry.LIGHT_ORB.get(), level);
            Vec3 pos = new Vec3(player.getX(), player.getY() + 1.5D, player.getZ());
            orb.moveOrInterpolateTo(pos, player.getYRot(), player.getXRot());
    ...
    protected Projectile createProjectile(Level level,
        ...
        if (ammoItem instanceof ArrowItem arrowItem
            ...
            arrow = new TorchbearerArrow(level, shooter, weaponStack, ammoStack);
        }
    ...
    public void appendHoverText(ItemStack stack,
        ...
        if (Minecraft.getInstance().hasShiftDown()) {
            tooltip.accept(Component.translatable("item.too_many_bows.torchbearer_bow.shift")
                    .withStyle(ChatFormatting.YELLOW));
            tooltip.accept(Component.translatable("item.too_many_bows.torchbearer_bow.effect")
                    .withStyle(ChatFormatting.RED));
        } else {
            tooltip.accept(Component.translatable("item.too_many_bows.torchbearer_bow")
