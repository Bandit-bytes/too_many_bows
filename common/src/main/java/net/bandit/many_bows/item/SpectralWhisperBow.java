...
public class SpectralWhisperBow extends ModBowItem {
    ...
    public boolean releaseUsing(ItemStack bowStack, Level level, LivingEntity entity, int chargeTime) {
        ...
        if (level.isClientSide()) {
            manybows$resetPullVisual(bowStack);
        }
    ...
    protected Projectile createProjectile(Level level, LivingEntity shooter, ItemStack weaponStack, ItemStack ammoStack, boolean crit) {
        ...
        if (ammoItem instanceof ArrowItem arrowItem
            ...
            arrow = new SpectralArrow(level, shooter, weaponStack, ammoStack);
        }
        ...
        if (shooter instanceof Player player) {
            ...
            double base = 2.0D;
            double attr = player.getAttributeValue(Attributes.ATTACK_DAMAGE);
            double power = getPowerFromBowDraw(player, weaponStack);

            double finalDamage = (base + attr) * 1.16D * power;
    ...
    private float getPowerFromBowDraw(Player player, ItemStack bowStack) {
        if (!player.isUsingItem()) return 1.0F;

        int remaining = player.getUseItemRemainingTicks();
        int rawCharge = this.getUseDuration(bowStack, player) - remaining;

        float mult = this.manybows$getChargeMultiplier(bowStack, player);
        int scaledCharge = Math.max(0, (int) (rawCharge * mult));

        return getPowerForTime(scaledCharge);
    }
    ...
    public void appendHoverText(ItemStack stack,
                                ...
                                TooltipFlag flag) {
        if (isShiftDownSafe()) {
            tooltip.accept(Component.translatable("item.too_many_bows.spectral_whisper").withStyle(ChatFormatting.AQUA));
            tooltip.accept(Component.translatable("item.too_many_bows.spectral_whisper.tooltip").withStyle(ChatFormatting.GRAY));
            tooltip.accept(Component.translatable("item.too_many_bows.spectral_whisper.details").withStyle(ChatFormatting.DARK_PURPLE));
        } else {
