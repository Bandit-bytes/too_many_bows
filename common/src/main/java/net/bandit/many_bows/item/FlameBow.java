...
public class FlameBow extends ModBowItem {
    ...
    protected Projectile createProjectile(Level level,
        ...
        if (ammoItem instanceof ArrowItem arrowItem
            ...
            arrow = new FlameArrow(level, shooter, weaponStack, ammoStack);

            if (arrow instanceof FlameArrow flame) {
                flame.setPowerMultiplier(1.0F);
            }
        }
    ...
    protected void shootProjectile(LivingEntity shooter,
        ...
        if (projectile instanceof FlameArrow flame) {
            float power = Math.min(1.0F, velocity / 2.5F);
            flame.setPowerMultiplier(power);
        }
    ...
    public void appendHoverText(ItemStack stack,
        ...
        if (isShiftDownSafe()) {
            tooltip.accept(Component.translatable("item.too_many_bows.flame_bow.tooltip").withStyle(ChatFormatting.RED));
            tooltip.accept(Component.translatable("item.too_many_bows.flame_bow.tooltip.ability").withStyle(ChatFormatting.GOLD));
            tooltip.accept(Component.translatable("item.too_many_bows.flame_bow.tooltip.legend").withStyle(ChatFormatting.DARK_RED, ChatFormatting.ITALIC));
        } else {
