...
public abstract class ModBowItem extends BowItem {
    ...
    protected void applyBowDamageAttribute(AbstractArrow arrow, Player player) {
        double attributeMultiplier = getBowDamageMultiplier(player);
        double configMultiplier = ManyBowsConfigHolder.get().globalBowPowerMultiplier;
        setArrowDamage(arrow, getArrowDamage(arrow) * attributeMultiplier * configMultiplier);
    }
    ...
    protected void damageBow(ItemStack bowStack, Player player, InteractionHand hand) {
        ...
        if (player.entityTags().contains(FLETCHERS_TALISMAN_TAG)) {
    ...
    protected float manybows$getChargeMultiplier(ItemStack stack, LivingEntity entity) {
        float basePullTicks = ManyBowsConfigHolder.get().globalBowPullSpeed;
