...
public class HunterBow extends BowItem {
                    ...
                    if (!projectiles.isEmpty() && level instanceof ServerLevel serverLevel) {
                        for (ItemStack projectileStack : projectiles) {
                            ...
                            if (projectileStack.is(Items.SPECTRAL_ARROW) || projectileStack.is(Items.TIPPED_ARROW)) {
                                ...
                                arrow = new HunterArrow(serverLevel, player, bowStack, projectileStack);
                                if (arrow instanceof HunterArrow hunterArrow) {
                                    ...
                                    if (rangedDamageAttr != null) {
                                        ...
                                        if (attrInstance != null) {
                                            ...
                                            hunterArrow.setBaseDamage(damage / 1.75);
                                        }
                                    }
                                }
                            }
    ...
    public void appendHoverText(ItemStack stack, TooltipContext context, List<Component> tooltipComponents, TooltipFlag tooltipFlag) {
        if (Screen.hasShiftDown()) {
            ...
            tooltipComponents.add(Component.translatable("item.too_many_bows.hunter_bow.mob3").withStyle(ChatFormatting.GREEN));
            tooltipComponents.add(Component.translatable("item.too_many_bows.hunter_bow.mob4").withStyle(ChatFormatting.GREEN));
            tooltipComponents.add(Component.translatable("item.too_many_bows.hunter_bow.mob5").withStyle(ChatFormatting.GREEN));
        } else {
            tooltipComponents.add(Component.translatable("item.too_many_bows.hunter_bow_bow").withStyle(ChatFormatting.GOLD));
            tooltipComponents.add(Component.translatable("item.too_many_bows.hunter_bow.tooltip").withStyle(ChatFormatting.GREEN));
            tooltipComponents.add(Component.translatable("item.too_many_bows.hunter_bow.hold_shift").withStyle(ChatFormatting.GRAY));
        }
    }
}
