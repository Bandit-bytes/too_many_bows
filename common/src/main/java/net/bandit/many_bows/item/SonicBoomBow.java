...
public class SonicBoomBow extends ModBowItem {
    ...
    public void releaseUsing(ItemStack bowStack, Level level, LivingEntity entity, int chargeTime) {
        if (entity instanceof Player player) {
            ...
            if (!arrowStack.isEmpty() || hasInfinity || player.getAbilities().instabuild) {
                    ...
                    if (!projectiles.isEmpty() && level instanceof ServerLevel serverLevel) {
                        for (ItemStack projectileStack : projectiles) {
                            ...
                            if (projectileStack.is(Items.SPECTRAL_ARROW) || projectileStack.is(Items.TIPPED_ARROW)) {
                                ...
                                arrow = new SonicBoomProjectile(serverLevel, player, bowStack, projectileStack);
                                if (arrow instanceof SonicBoomProjectile sonicBoomProjectile) {
                                    ...
                                    if (rangedDamageAttr != null) {
                                        ...
                                        if (attrInstance != null) {
                                            ...
                                            sonicBoomProjectile.setBaseDamage(damage / 2);
                                        }
                                    }
                                    sonicBoomProjectile.setPowerMultiplier(power);
                                }
                            }
    ...
    public void appendHoverText(ItemStack stack, TooltipContext context, List<Component> tooltipComponents, TooltipFlag tooltipFlag) {
        if (Screen.hasShiftDown()) {
            tooltipComponents.add(Component.translatable("item.too_many_bows.sonic_bow.tooltip.ability").withStyle(ChatFormatting.AQUA));
            tooltipComponents.add(Component.translatable("item.too_many_bows.sonic_bow.tooltip.legend").withStyle(ChatFormatting.DARK_PURPLE, ChatFormatting.ITALIC));
        } else {
            tooltipComponents.add(Component.translatable("item.too_many_bows.sonic_bow.tooltip").withStyle(ChatFormatting.GOLD));
