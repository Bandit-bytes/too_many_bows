...
public class SentinelBow extends ModBowItem {
    ...
    protected Projectile createProjectile(Level level, LivingEntity shooter, ItemStack weaponStack, ItemStack ammoStack, boolean crit) {
        ...
        if (ammoItem instanceof ArrowItem arrowItem
            ...
            arrow = new SentinelWrathArrow(level, shooter, weaponStack, ammoStack);
        }
        ...
        if (shooter instanceof Player player) {
            ...
            applyRangedWeaponDamageBase(level, player, arrow);

            applyBowDamageAttribute(arrow, player);
            tryApplyBowCrit(arrow, player, 1.5D);
        }

        if (arrow instanceof SentinelWrathArrow sentinel) {
            sentinel.setPowerMultiplier(1.0F);
        }

        return arrow;
    }

    private void applyRangedWeaponDamageBase(Level level, Player player, AbstractArrow arrow) {
    ...
    protected void shootProjectile(LivingEntity shooter, Projectile projectile, int index,
        ...
        if (projectile instanceof SentinelWrathArrow sentinel) {
            sentinel.setPowerMultiplier(velocity / 2.5F);
        }
    ...
    public void appendHoverText(ItemStack stack,
        ...
        if (Minecraft.getInstance().hasShiftDown()) {
            tooltip.accept(Component.translatable("item.too_many_bows.sentinels_wrath.shift_title")
                    .withStyle(ChatFormatting.YELLOW));
            tooltip.accept(Component.translatable("item.too_many_bows.sentinels_wrath.mob1").withStyle(ChatFormatting.GREEN));
            tooltip.accept(Component.translatable("item.too_many_bows.sentinels_wrath.mob2").withStyle(ChatFormatting.GREEN));
            tooltip.accept(Component.translatable("item.too_many_bows.sentinels_wrath.mob3").withStyle(ChatFormatting.GREEN));
            tooltip.accept(Component.translatable("item.too_many_bows.sentinels_wrath.mob4").withStyle(ChatFormatting.GREEN));
            tooltip.accept(Component.translatable("item.too_many_bows.sentinels_wrath.mob5").withStyle(ChatFormatting.GREEN));
            tooltip.accept(Component.translatable("item.too_many_bows.sentinels_wrath.mob6").withStyle(ChatFormatting.GREEN));
        } else {
            tooltip.accept(Component.translatable("item.too_many_bows.sentinels_wrath.tooltip")
                    .withStyle(ChatFormatting.GOLD));
            tooltip.accept(Component.translatable("item.too_many_bows.sentinels_wrath.hold_shift")
