                    ...
                    if (!projectiles.isEmpty() && level instanceof ServerLevel serverLevel) {
                        for (ItemStack projectileStack : projectiles) {
                            ...
                            if (projectileStack.is(Items.SPECTRAL_ARROW) || projectileStack.is(Items.TIPPED_ARROW)) {
                                ...
                                if (arrow instanceof SentinelWrathArrow sentinelWrathArrow) {
                                    ...
                                    if (rangedDamageAttr != null) {
                                        ...
                                        if (attrInstance != null) {
                                            ...
                                            sentinelWrathArrow.setBaseDamage(damage / 2.5);
                                        }
                                    }
                                    sentinelWrathArrow.setPowerMultiplier(power);
                                }
                            }
...
tryApplyBowCrit(arrow, player, 1.5D);
    ...
    public void appendHoverText(ItemStack stack, TooltipContext context, List<Component> tooltipComponents, TooltipFlag tooltipFlag) {
        if (Screen.hasShiftDown()) {
            tooltipComponents.add(Component.translatable("item.too_many_bows.sentinels_wrath.shift_title").withStyle(ChatFormatting.YELLOW));
            tooltipComponents.add(Component.translatable("item.too_many_bows.sentinels_wrath.mob1").withStyle(ChatFormatting.GREEN));
            tooltipComponents.add(Component.translatable("item.too_many_bows.sentinels_wrath.mob2").withStyle(ChatFormatting.GREEN));
            tooltipComponents.add(Component.translatable("item.too_many_bows.sentinels_wrath.mob3").withStyle(ChatFormatting.GREEN));
            tooltipComponents.add(Component.translatable("item.too_many_bows.sentinels_wrath.mob4").withStyle(ChatFormatting.GREEN));
            tooltipComponents.add(Component.translatable("item.too_many_bows.sentinels_wrath.mob5").withStyle(ChatFormatting.GREEN));
            tooltipComponents.add(Component.translatable("item.too_many_bows.sentinels_wrath.mob6").withStyle(ChatFormatting.GREEN));
        } else {
            tooltipComponents.add(Component.translatable("item.too_many_bows.sentinels_wrath.tooltip").withStyle(ChatFormatting.GOLD));
            tooltipComponents.add(Component.translatable("item.too_many_bows.sentinels_wrath.hold_shift").withStyle(ChatFormatting.GRAY));
        }
    }
}
